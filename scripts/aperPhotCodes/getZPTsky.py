...
def outZPTsky(targname,filt,dataDir,resDir):
    with fits.open(os.path.join(dataDir,f'{targname}_f{filt}w.fits')) as hdu:
        expend = Time(hdu[0].header['EXPEND'],format='mjd')
        dateI = expend.to_value('iso',subfmt='date')
        full_name = hdu[0].header['TARGNAME']
        
    im,_ = io.readfile(os.path.join(resDir,f'{targname}_f{filt}w.fits'))
    try:
        _, skysig = sky.getsky(im)
    except ValueError:
        _, skysig = sky.getsky(im[0])  # The im tends to have 3 dimensions, for some reason

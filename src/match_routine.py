...
def mk_plot(x1, y1, x2, y2, lab1, lab2, outname='None', save_dir='./'):
    ...
    plt.savefig(os.path.join(
        save_dir, f'{outname}.pdf'), bbox_inches='tight', dpi=150)
    plt.close()

    return None


def match_in(prime, cat, matchtol, xpri, ypri, xcat, ycat, idcat):
    """
    Input:
    ...
    while nF:
        ...
        # source index into the matchids array.
            ...
            dist_diff = np.zeros((len(matchrows), 1))
            for dd in range(len(matchrows)):
                dist_diff[dd] = np.sqrt((prime[f'{xpri}'][row]
                                         - matchrows[f'{xcat}'][dd])**2
                                        + (prime[f'{ypri}'][row]
                                           - matchrows[f'{ycat}'][dd])**2)
            small = np.argsort(dist_diff)
            idx = small[0]  # The index of the smallest distance in dist_diff
            matchids_in[row][0] = matchrows[f'{idcat}'][idx]
            prime['dist'] = dist_diff[idx]
            row += 1
        ...
        if (row >= len(prime)):
            match_id_out, udx = np.unique(matchids_in, return_index=True)
...
def matchlist_id(prime, cat, matchtol, xpri, ypri, xcat, ycat, idcat, tag='', save_dir='./', plot=False):

    p_out, pids, uu = match_in(
        prime, cat, matchtol, xpri, ypri, xcat, ycat, idcat)
    ...
    if plot:
        mk_plot(p_out[f'{xpri}'], p_out[f'{ypri}'],
                cat[pidx][f'{xcat}'], cat[pidx][f'{ycat}'],
                lab1='Prime1', lab2='Cat1', outname=f'aper_run1{tag}',
                save_dir=save_dir)
    ...
    if uu == 0:
        ...
        # Length of unique cat values that had matches in prime
        print(f'Unique cat values: {len(pids)}')
        ...
        # Getting only the unique cat values that had a match in p_out (shortened prime);
        p2 = cat[pidx]
        ...
        while nF:
            if iter % 2 == 0:
                ...
                # p2 is from CAT, c2 is from PRIME
                c_out, cids, uu = match_in(
                    p2, c2, matchtol, xcat, ycat, xpri, ypri, 'id_u')
                ...
                if plot:
                    mk_plot(c_out[f'{xcat}'], c_out[f'{ycat}'], c2[f'{xpri}'], c2[f'{ypri}'],
            ...
            # cids are the index values of c2 (copy of p_out) that match with sources in c_out
                ...
                # p2 is from PRIME, c2 is from CAT
                c_out, cids, uu = match_in(
                    p2, c2, matchtol, xpri, ypri, xcat, ycat, 'id_u')
                cidx = np.asarray(cids, dtype=int)
            ...
            if uu == 0 and iter % 2 == 0:  # uu has gotten to be 0 from the above if/else, even loop; can output and end
                ...
                print(f'ITER {iter:d}')
                # Individual prime values, 2 cuts deep, last copy of c2, which would be from PRIME
                p2 = deepcopy(c2[cidx])
                # To be used in matching algorithm with c2 input
                c_out['id_u'] = np.arange(0, len(c_out), 1, dtype=int)
                # Copy of cat, 2 cuts deep, with a two cat values that match to one prime value
                c2 = deepcopy(c_out)
                iter += 1

            elif uu == 1:  # If still problem, but odd iteration, send to even loop, but switch prime and cat
                s_cat = cat[s_cat_idx]  # Shortened cat
                p2 = deepcopy(s_cat)
                c2 = c_out

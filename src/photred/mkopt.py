...
def mkopt(inpfiles,hilimit=6.4e4,va=2,fitradius_fwhm=None,
          inp_fwhm=None,verbose=True,scale=0.035):
    """
    ...
    Edited for HST files by HCR, Nov 2023
    """ 
    ...
    # If the image had scaled final pixfrac:
    if scale is not None:
        scale = scale/0.05 # plate scale
    else:
        scale = 1.0
    ...
    # Getting READNOISE 
    rdnoise1,rdnoisekey = io.getrdnoise(filename)    
             
    rdnoise = np.sqrt((rdnoise1*scale)**2 + skysig**2)
    print(f'SCALED READNOISE IS {rdnoise:.2f}')
             
    # Getting saturation limit from the header 
    lolimit = 10000.0  # just in case 
    saturate = head.get('GOODMAX')
    ...
    if verbose:
        ...
        print(f'rdnoise = {rdnoise:.2f}') 
        print(f'fwhm = {fwhm:.2f}') 
        print(f'saturation = {hi:.2f}')

...
def fileinfo(files):
    ...
    for i in range(nfiles): 
        ...
        if files[i][-7:]=='fits.fz':
            head = readfile(files[i],exten=1,header=True)
...
def getgain(filename=None,head=None):
    ...
    if gain is None:
        gain_arr = np.empty(4)
        gain_arr[0] = head.get('ATODGNA')
        gain_arr[1] = head.get('ATODGNB')
        gain_arr[2] = head.get('ATODGNC')
        gain_arr[3] = head.get('ATODGND')
    
    gain = np.mean(gain_arr)
    print(f'GAIN is {gain:.2f}')
...
def getrdnoise(filename=None,head=None):
    ...
    if rdnoise is None and readnois is None and enoise is None:
        rd_narr = np.empty(4)
        rd_narr[0] = head.get('READNSEA')
        rd_narr[1] = head.get('READNSEB')
        rd_narr[2] = head.get('READNSEC')
        rd_narr[3] = head.get('READNSED')

        readnoise = np.mean(rd_narr)
        keyword = None
        print(f'READNOISE IS {readnoise:.2f}')

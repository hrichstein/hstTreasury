...
def getsky(image, silent=False, circlerad=False, meanback=False, highbad=None,
    ...
    # Maximum # of pixels to be used in sky calculation
    maxsky = np.maximum(2*npts//(nrow-1), 10000)
    ...
    if nstep > 1:
        i0 = np.maximum((nrow-1 - max(index0) - istep)//2,
                        0)  # Adjust margin for symmetry
        index0 = index0 + i0
    ...
    if meanback:
        ...
        skymode, skysig, skynew, nsky = utils.mmm(
            skyvec, readnoise=readnoise, integer=integer, highbad=highbad)
